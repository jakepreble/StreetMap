    ...
    private int nearestUnvisitedVertex(HashMap<Integer, VertexInfo> info, HashSet<Integer> unvisited) {
        //YOUR CODE HERE:
         ...
         for(int i = 0; i < unvisited.size(); i++){
            if(info.get(i).distance() < minDistance){
                nearestKey = i;
                minDistance = info.get(nearestKey).distance();
            }
         } 
         return nearestKey;
...
public void shortestPath(int start, int end, Color color) {
        ...
        while(true) {
            ...
            //SOLUTION:
            if(info.get(current).distance() == Double.POSITIVE_INFINITY) return;
            ...
            for(Edge<Integer, Double> edge : this.getAdjacencyList(current)) {
                ...
                double alternative = info.get(current).distance() + edge.getEdgeData();
                System.out.println("Original:  " + info.get(vertex).distance());
                System.out.println("alt: " + alternative);
                if(alternative < info.get(vertex).distance()) {
